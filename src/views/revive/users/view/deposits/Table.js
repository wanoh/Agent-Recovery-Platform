...
const WithdrawalTransactions = () => {
  ...
  const dataToRender = () => {
    ...
    if (store.transactionsData.length > 0) {
      const depositTransactions = store.transactionsData.filter(
      ...
      const depositTransactions = store.allTransactionsData.filter(
        (transaction) => transaction.type === 'Deposit'
      )
      console.log('depositTransactions', depositTransactions)
      return depositTransactions.slice(0, rowsPerPage)
    }
  }
